...
from src.backend.analysis import load_prebaked_curated_correspondences
...
class ComparisonReportTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        for example in curated.list_examples():
            ...
            cls.examples[example] = (timeline, load_prebaked_curated_correspondences(example, timeline))

    def test_span_order_does_not_change_the_report(self) -> None:
        for example, (timeline, overlays) in self.examples.items():
            for lower, higher in SPANS:
                with self.subTest(example=example, span=(lower, higher)):
                    self.assertEqual(describe_comparison(timeline, overlays, lower, higher),
                                     describe_comparison(timeline, overlays, higher, lower))

    def test_report_covers_exactly_the_states_and_steps_in_the_span(self) -> None:
        for example, (timeline, overlays) in self.examples.items():
            for lower, higher in SPANS:
                with self.subTest(example=example, span=(lower, higher)):
                    report = describe_comparison(timeline, overlays, higher, lower)
    ...
    def test_every_item_is_backed_by_resolvable_evidence(self) -> None:
        for example, (timeline, overlays) in self.examples.items():
            for lower, higher in SPANS:
                with self.subTest(example=example, span=(lower, higher)):
                    report = describe_comparison(timeline, overlays, lower, higher)
    ...
    def test_adjacent_span_reports_the_stored_overlay(self) -> None:
        for example, (timeline, overlays) in self.examples.items():
            for ordinal in range(len(overlays)):
                with self.subTest(example=example, ordinal=ordinal):
                    report = describe_comparison(timeline, overlays, ordinal, ordinal + 1)
                    self.assertEqual(report.links, overlays[ordinal].links)

    def test_composed_span_says_so(self) -> None:
        for example, (timeline, overlays) in self.examples.items():
            with self.subTest(example=example):
                report = describe_comparison(timeline, overlays, 0, 13)
    ...
    def test_same_state_reports_no_change(self) -> None:
        for example, (timeline, overlays) in self.examples.items():
            for ordinal in range(len(timeline.states)):
                with self.subTest(example=example, ordinal=ordinal):
                    report = describe_comparison(timeline, overlays, ordinal, ordinal)
                    self.assertEqual(report.context, SAME_STATE_CONTEXT)
                    self.assertEqual([s.ordinal for s in report.states], [ordinal])
                    self.assertEqual((report.steps, report.links, report.items, report.optimisations),
                                     ((), (), (), ()))

    def test_ordinal_outside_the_timeline_is_rejected(self) -> None:
        timeline, overlays = next(iter(self.examples.values()))
        with self.assertRaises(ValueError):
            describe_comparison(timeline, overlays, 0, len(timeline.states))
