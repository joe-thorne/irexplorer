...
- **S1.5** — deterministic, coverage-complete endpoint `Correspondence` overlays with exact/approximate confidence, audit evidence, and concise link-backed summaries.
- **S1.6** — read-only `localhost` query API (`api/query.py`, `api/server.py`) over the pre-baked model records, with IR/CFG/navigation/counterpart/summary queries and in-session focus retention. The endpoint MVP uses the two-state `-O0`/recompiled-`-O3` timeline.
...
- **S2.1** — eager `valueFlow` def-use edges and derived indices, plus a deterministic hybrid matcher. Exact instruction links require unique structural, source, and value-flow agreement; source-anchored and positional links are approximate; inspected but unresolved candidates surface as explicit `none` links. Endpoint records were re-baked through the existing offline path and checksum-gated.

Next: S2.2 — persist adjacent overlays for the full teaching-pass timeline, add correspondence composition, and introduce the recompiled-anchor mode. Outstanding issues are tracked inline in the implementation plan.
