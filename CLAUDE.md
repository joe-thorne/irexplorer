...
Next: review E4 in `../Docs/evaluation/e4-review.md` before E5. Joe reviewed E0–E3 with no amendments, including E3's source-loading fix. No instrument correction was explicitly accepted. E2/E3 source-aware comparison controls remain. E4 replaces the synthetic form placeholders with participant-only information/consent, P1–P13 and Q1–Q20, inline validation, tab-local recovery after consent, explicit storage failure handling, P13 locking with background corrections, and local response review. The separate `src/backend/evaluation/` definition/validator and GET `/api/study/content` do not collect responses. The task stage remains a labelled preview; E5 owns T0–T6/timing and E6 owns submission/storage. Preview: `http://127.0.0.1:8000/?revision=e4-forms-1#/study` or `/#/explore`. Verification: 63 backend tests, 12 draft checks, 78 browser assertions, and seven inspected captures; see `docs/evaluation-captures/e4-forms.md`. Instrument v0.1, E0 mapping, model and canonical artefacts are unchanged. Physical keyboard/spoken screen-reader, broader accessibility and S1.8 remain E7. Live arbitrary input remains Phase 5 only after the S2.6 activation gate.
...
- `src/backend/evaluation/` — participant-only consent/survey definition and validation; response collection remains E6.
