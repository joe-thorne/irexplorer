...
Next: review E2 in `../Docs/evaluation/e2-review.md` before E3. Joe reviewed E0 and E1 with no amendments; no instrument correction was explicitly accepted. E2 exposes verified canonical C and function-scoped debug-location mappings through typed GET routes, with bidirectional source/IR/CFG highlighting, retained anchors, and asynchronous guards. E1's synthetic study shell remains; actual forms, timing, drafts, and response collection are later work. E3–E8 are not implemented. Preview: `http://127.0.0.1:8002/#/explore` and `/#/study` on the same port. Verification: 31 browser assertions and 55 backend tests pass; see `docs/evaluation-captures/e2-source.md`. CFG defects remain E3; physical-keyboard/wider accessibility and S1.8 remain E7. Instruments and canonical artefacts are unchanged. Live arbitrary input remains Phase 5 only after the S2.6 activation gate.
