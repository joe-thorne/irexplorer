...
window.StudyDraft = (() => {
  const KEY = 'irexplorer.study.e4';
  const blank = () => ({ status: 'unanswered', value: null });
  const fieldsFor = (content, stage) => content.fields.filter(f => f.id.startsWith(stage === 'pre' ? 'P' : 'Q'));
  ...
  function validate(content, stage, answers, complete = false, recovering = false) {
    ...
    for (const field of fields) {
      ...
      if (status === 'not_applicable' && value === null && (field.notApplicableLabel || field.optionStatuses)) continue;
      ...
      if (!valid) errors[field.id] = field.type === 'text' ? `Use no more than ${field.maxLength.toLocaleString()} characters. Your text has not been shortened.` : 'Choose one of this item’s options.';
    }
    return errors;
  }
  function create(content, acknowledgements) {
    ...
    return { schemaVersion: 1, studyVersion: content.studyVersion, contentVersion: content.contentVersion,
      ...
      preComplete: false, p13Locked: false, previewTaskComplete: false, reviewReady: false };
  }
  function decode(raw, content) {
    if (raw.length > 256 * 1024) throw new Error('Draft is too large.');
    const d = JSON.parse(raw);
    const keys = ['schemaVersion', 'studyVersion', 'contentVersion', 'instrumentVersion', 'participantCode', 'consent', 'pre', 'post', 'preComplete', 'p13Locked', 'previewTaskComplete', 'reviewReady'];
    if (!d || Object.keys(d).sort().join() !== keys.sort().join() || d.schemaVersion !== 1 || d.studyVersion !== content.studyVersion || d.contentVersion !== content.contentVersion || d.instrumentVersion !== content.instrumentVersion || !/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(d.participantCode)) throw new Error('Unsupported draft.');
    ...
    if (!['preComplete', 'p13Locked', 'previewTaskComplete', 'reviewReady'].every(k => typeof d[k] === 'boolean') || (d.preComplete && !d.p13Locked) || (d.previewTaskComplete && !d.p13Locked) || (d.reviewReady && !d.previewTaskComplete)) throw new Error('Invalid progress.');
  ...
  return { KEY, blank, fieldsFor, visible, validate, create, decode, storage };
})();
