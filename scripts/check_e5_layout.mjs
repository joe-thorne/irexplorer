// E5 behavioural verification. Node 22+ built-ins; isolated local headless Chrome on :9228.
...
if (!page) throw new Error('Start an isolated headless Chrome at about:blank on :9228.');
...
const prefix=baseline ? preventFocusScroll ? 'e5-layout-before-focus' : 'e5-layout-before' : 'e5-layout-after';
...
try {
  ...
  if(!baseline) {
    ...
    await check('Compact graphs reduce vertical spacing', `document.querySelector('#right-viewer svg').viewBox.baseVal.height < 850`);
      ...
      for(let ordinal=0;ordinal<14;ordinal++) {
        ...
        for(const name of functions) {
          ...
          await check(`Compact geometry ${example}/${ordinal}/${name}: labels, nodes and edges remain contained`, `['left','right'].every(side=>{
            ...
            return [...s.querySelectorAll('.cfg-node')].every(n=>n.querySelector('text').getComputedTextLength()<=Number(n.querySelector('rect').getAttribute('width'))-20) &&
